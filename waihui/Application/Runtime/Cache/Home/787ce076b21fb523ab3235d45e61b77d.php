...
<div class="main_box">
    ...
    <ul>
        ...
        <li><a href="/index.php/Home/Login/logout">退出<i class="icon_right"></i></a></li>
    </ul>
</div>
    <!-- 中间主体 -->
    <div class="usermain right_userpay">
        <div class="chongzhi">
            ...
            <br>
            <div class="bank">
                <?php if(is_array($bankCards)): $i = 0; $__LIST__ = $bankCards;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bankCard): $mod = ($i % 2 );++$i;?><span><?php echo ($bankCard["bank_card_num"]); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
            </div>

            <br>
            <br>
            <form id="form">
                <input type="hidden" name="recharge_id" value="<?php echo ($recharge["recharge_id"]); ?>">
                <div class="ul_title"><span></span>充值金额</div>
                <br>
                <div class="form-inline">
                    <input type="number" name="money" class="form-control" value="<?php echo ($recharge["money"]); ?>">
                </div>
                <br>
                <div class="ul_title"><span></span>汇款信息</div>
                <br>
                <textarea name="detail" class="form-control" rows="3"><?php echo ($recharge["detail"]); ?></textarea>
                <br>
                <div class="text-center">
                    <button class="btn btn-primary" type="button" onclick="edit()">重新提交审核</button>
                </div>
                <br>
                <div class="text-right">
                    <button class="btn btn-info" type="button" onclick="location.href = '/index.php/Home/User/allrecharge'">查看充值记录</button>
                </div>
            </form>
        </div>
    </div>
            ...
            success: function (res) {
                if (res.code === 0) {
                    layer.open({
                        ...
                        content: res.msg,
                        yes: function () {
                            location.href = '/index.php/Home/User/allrecharge';
                        }
                    });
                } else {
                    layer.open({
