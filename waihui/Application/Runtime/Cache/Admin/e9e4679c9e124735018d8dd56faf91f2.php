...
<div id="wrapper">
    ...
    <div class="main">
        ...
        <div class="main-content">
            <div class="container-fluid">
                <h3 class="page-title">银行卡管理</h3>
                <div>
                    <!-- BORDERED TABLE -->
                    <div class="panel">
                        <div class="panel-heading">
                            <div class="row">
                                <div class="col-sm-6"><h3 class="panel-title">银行卡信息表</h3></div>
                                <div class="col-sm-6 text-right">
                                    <span class="btn btn-primary" data-target="#addBankCard" data-toggle="modal"><i class="fa fa-plus-square"></i> 添加银行卡</span>
                                </div>
                            </div>
                        </div>
                        <div class="panel-body">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                <?php if(is_array($bankCards)): $i = 0; $__LIST__ = $bankCards;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$bankCard): $mod = ($i % 2 );++$i;?><tr class="user-table-font-size">
                                    <td><?php echo ($bankCard["bank_card_num"]); ?></td>
                                    <td><?php echo ($bankCard["intro"]); ?></td>
                                    <td>
                                        <button type="button" class="btn btn-danger" data-id="<?php echo ($bankCard["card_id"]); ?>" title="删除银行卡信息" onclick="delData(this)">
...
<div id="addBankCard" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="addAdmin">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <div class="modal-body">
                <input id="bankCardNum" class="form-control" placeholder="银行卡号" type="text">
                <br>
                <textarea id="intro" class="form-control" placeholder="银行卡信息描述"></textarea>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" onclick="addBankCard()" data-dismiss="modal">添加</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    const SCOPE = {
        ...
        deleteDataUrl: '/Admin/BankCard/del',
        addDataUrl: '/Admin/BankCard/add'
    };

    function addBankCard() {
        var data = {
            'bank_card_num': $("#bankCardNum").val(),
            'intro': $("#intro").val()
        };
