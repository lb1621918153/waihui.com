                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <div class="row">
                                ...
                                <div class="col-sm-6 text-right">
                                    <a href="/Admin/Product/Add" class="btn btn-primary">添加产品信息</a>
                                </div>
                            </div>
                        </div>
                        <div class="panel-body">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                <?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$product): $mod = ($i % 2 );++$i;?><tr class="user-table-font-size">
                                    <td><?php echo ($product["product_name"]); ?></td>
                                    <td><?php echo ($product["current"]); ?></td>
                                    <td><?php echo ($product["lowest"]); ?></td>
                                    <td><?php echo ($product["highest"]); ?></td>
                                    <td><?php echo ($product["open"]); ?></td>
                                    <td><?php echo ($product["income"]); ?></td>
                                    <td><?php echo ($product["handing_fee"]); ?>%</td>
                                    <td>
                                        <?php if($product["enable"] == 1): ?><button data-id="<?php echo ($product["id"]); ?>" onclick="disenable(this)" class="btn btn-success" title="当前前台用户可见">显示</button>
                                            <button data-id="<?php echo ($product["id"]); ?>" onclick="enable(this)" class="btn btn-danger hidden" title="当前前台用户不可见">隐藏</button><?php endif; ?>
                                        <?php if($product["enable"] == 0): ?><button data-id="<?php echo ($product["id"]); ?>" onclick="disenable(this)" class="btn btn-success hidden" title="当前前台用户可见">显示</button>
                                            <button data-id="<?php echo ($product["id"]); ?>" onclick="enable(this)" class="btn btn-danger" title="当前前台用户不可见">隐藏</button><?php endif; ?>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-danger" data-id="<?php echo ($product["id"]); ?>" data-toggle="modal" data-target="#writeIntro" title="删除后不可找回" onclick="delData(this)">
                                            删除
                                        </button>
                                        <button type="button" class="btn btn-primary" data-id="<?php echo ($product["id"]); ?>" onclick="showData(this)">
                                            编辑
                                        </button>
                                    </td>
...
<script>
    const SCOPE = {
        ...
        enableUrl: '/Admin/Product/enable',
        disenableUrl: '/Admin/Product/disenable'
    };
