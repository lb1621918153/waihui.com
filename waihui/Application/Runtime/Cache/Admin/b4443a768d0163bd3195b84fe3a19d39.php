                ...
                <div>
                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <div class="row">
                                <h3 class="panel-title">用户实名信息认证表</h3>
                            </div>
                        </div>
                        <div class="panel-body">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                <?php if(is_array($verifyUsers)): $i = 0; $__LIST__ = $verifyUsers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr class="user-table-font-size">
                                    <td><?php echo ($user["id"]); ?></td>
                                    <td><a href="/Admin/User/verify/id/<?php echo ($user["id"]); ?>"><?php echo ($user["username"]); ?></a></td>
                                    <td><?php echo ($user["true_name"]); ?></td>
                                    <td><?php echo ($user["true_id"]); ?></td>
                                    <td>
                                        <button type="button" class="btn btn-danger" data-id="<?php echo ($user["id"]); ?>" title="" onclick="refuseVerify(this)">
                                            拒绝
                                        </button>
                                        <button type="button" class="btn btn-primary" data-id="<?php echo ($user["id"]); ?>" title="" onclick="doVerify(this)">
                                            确认
                                        </button>
                                    </td>
...
<script>
    const SCOPE = {
        ...
        doVerifyUrl: '/Admin/User/doVerify',
        refuseVerifyUrl: '/Admin/User/refuseVerify',
    };

    function doVerify(ref) {
        var id = $(ref).attr('data-id');
        $.post(SCOPE.doVerifyUrl, {id: id}, function (res) {
            if (res.code === 0) {
                location.reload();
            }
            showTip(res.msg);
    ...
    function refuseVerify(ref) {
        var id = $(ref).attr('data-id');
        $.post(SCOPE.refuseRechargeUrl, {id: id}, function (res) {
            if (res.code === 0) {
                location.reload();
            }
            showTip(res.msg);
