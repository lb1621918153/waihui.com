                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <div class="row">
                                ...
                                <div class="col-sm-6 text-right">
                                    <a <?php if($verifyCount): ?>href="/Admin/User/verify"<?php endif; ?> class="btn btn-primary" type="button">
                                        实名认证请求 <span class="badge"><?php echo ($verifyCount); ?></span>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="panel-body">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                <?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr class="user-table-font-size">
                                    <td><?php echo ($user["id"]); ?></td>
                                    <td><?php echo ($user["username"]); ?></td>
                                    <td><?php echo ($user["phone"]); ?></td>
                                    <td><?php echo (showUserTrueName($user["true_name"])); ?></td>
                                    <td>￥<?php echo ($user["money"]); ?></td>
                                    <td><?php echo (showDateTime($user["login_time"])); ?></td>
                                    <td>
                                        <button type="button" class="btn btn-danger" data-id="<?php echo ($user["id"]); ?>" title="删除该用户" onclick="delData(this)">
                                            <i class="fa fa-trash" aria-hidden="true"></i> 删除
                                        </button>
                                        <button type="button" class="btn btn-primary" data-id="<?php echo ($user["id"]); ?>" title="查看用户详情" onclick="showData(this)">
...
<script>
    const SCOPE = {
        ...
        deleteDataUrl: '/Admin/User/del',
        showDataUrl: '/Admin/User/detail'
    };
</script>
</body>
</html>